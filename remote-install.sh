...
# ============================================================================
# macOS Development Environment Remote Setup Script
# ============================================================================
# Complete self-contained setup script that can be run remotely
# Usage: curl -fsSL https://raw.githubusercontent.com/arthurdaquinosilva/dotfiles/main/remote-install.sh | bash
# ============================================================================
...
# Configuration
GITHUB_USER="arthurdaquinosilva"
DOTFILES_REPO="git@github.com:$GITHUB_USER/dotfiles.git"
VIM_REPO="git@github.com:$GITHUB_USER/vim.git"
...
install_homebrew() {
    ...
    bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)" < /dev/tty
...
setup_github_ssh() {
    ...
    # Always set/update git config first
...
# Clone repositories
clone_repositories() {
    log_info "Cloning dotfiles and vim repositories..."
    ...
    if [[ ! -d "$DOTFILES_DIR" ]]; then
        ...
        git clone "$DOTFILES_REPO" "$DOTFILES_DIR" || {
            log_error "Failed to clone dotfiles repository. Make sure your SSH key is set up correctly."
            return 1
        }
    ...
    if [[ ! -d "$VIM_DIR" ]]; then
        ...
        log_success "Vim repository cloned successfully"
    else
        log_warning "Vim directory already exists"
    fi
    ...
    log_info "Changed to dotfiles directory"
}
...
install_oh_my_zsh() {
    ...
    if [[ ! -d "$HOME/.oh-my-zsh" ]]; then
        ...
        # Install Oh My Zsh
        sh -c "$(curl -fsSL https://raw.github.com/ohmyzsh/ohmyzsh/master/tools/install.sh)" "" --unattended < /dev/tty
...
setup_vim_plugins() {
    ...
    $VIM_PATH -c 'PlugInstall --sync' -c 'qa' < /dev/tty
...
configure_fzf() {
    ...
    /opt/homebrew/opt/fzf/install --all --no-bash --no-fish < /dev/tty
...
main() {
    log_info "============================================================================"
    log_info "🚀 macOS Development Environment Remote Setup"
    log_info "============================================================================"
    log_info "This will set up your complete development environment remotely"
    log_info ""

    # Confirm with user
    echo -e "${YELLOW}Do you want to proceed with the remote installation? (y/N):${NC}"
    ...
    log_success "============================================================================"
    log_info "Your complete development environment is now ready!"
    log_info "Please restart your terminal or run 'source ~/.zshrc' to activate all changes."
}
