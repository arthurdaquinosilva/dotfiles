...
# Wait for any running Homebrew process to finish; remove orphaned lock files if brew is not running
wait_for_brew_lock() {
    ...
    while true; do
        ...
        if pgrep -x brew >/dev/null 2>&1; then
            if [[ $waited -eq 0 ]]; then
                log_warning "Another Homebrew process is running. Waiting up to ${max_wait}s..."
            fi
            sleep 10
            waited=$((waited + 10))
            if [[ $waited -ge $max_wait ]]; then
                log_error "Timed out waiting for Homebrew lock. Please terminate the other brew process and retry."
                exit 1
            fi
        else
            log_warning "Orphaned Homebrew lock files found (brew is not running). Removing them..."
